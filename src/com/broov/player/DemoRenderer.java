...
class DemoRenderer extends GLSurfaceView_SDL.Renderer {
	...
	public void onDrawFrame(GL10 gl) 
	{
		...
		if (Globals.getNativeVideoPlayer()) {
		    try {
		        result = nativeVideoPlayerInit("/sdcard/broov.ttf", 0, 11, 0, 1);
		    } catch (Exception e) {
		        e.printStackTrace();
		    }
		} else {
		    result = nativePlayerInit(Globals.dbSubtitleFont, FileManager.getshow_subtitle(), FileManager.getSubTitleSize(), Globals.dbSubtitleEncoding, rgb565);
		}
		
		VideoInfo info = VideoPlayer.getNextVideoItem();
		Globals.fileName = info.strVideoLocalPath;
		...
		if (Globals.getNativeVideoPlayer()) {
			...
			if ((audioFileType == 1) && FileManager.isAudioStream(Globals.fileName)){
				...
				retValue = nativePlayerMain(Globals.fileName, loopselected, audioFileType, 
			        ...
			        System.out.println("Native player exception");
			    }
			}
		}
		...
		while (!usergeneratedexitApp && playnextfileFromDirectory ){
			if (retValue == 100){
				info = VideoPlayer.getPrevVideoItem();
				fileInfoUpdated = true;
			}else{
				info = VideoPlayer.getNextVideoItem();
				fileInfoUpdated = true;
			}
			...
			System.out.println("nextFile before:"+Globals.fileName);
			...
			System.out.println("nativePlayerMain(fileName:"+Globals.fileName+", loopselected:"+loopselected+", audioFileType: "+audioFileType+");");
